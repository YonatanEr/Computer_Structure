...
void download_memin_to_ram() { //downloads the entirety of memin to a char** ram.
	...
	for (int i = 0; i < MEM_MAX_SIZE; i++) {
		char memin_line[INSTRUCTION_BYTES+1];
		fscanf(fptr_memin, "%s", &memin_line);
		ram[i] = hex_string_to_int_signed(memin_line);
		//printf("%d", ram[i]);
	}
	fclose(fptr_memin);
}
...
void simulator() {
	...
	for (int halt = 0; !halt; trace_line[0] = next_pc) { //while halt was not asserted, update to the next PC and and execute its command.
